// implement backspace button bro 
package main
...
func main() {
	...
	router.HandleFunc("/place-order", placeOrderHandler).Methods("POST")
...
func addToOrderHandler(w http.ResponseWriter, r *http.Request) {
	...
	dbnama = append(dbnama, foodItem.Name)
	dbharga = append(dbharga, foodItem.Price)
...
func placeOrderHandler(w http.ResponseWriter, r *http.Request) {
    ...
    // For example:
	for i := 0 ; i<len(dbnama) ; i++{
		_, err := db.Exec("INSERT INTO orders (Makanan, Harga) VALUES (?, ?)", dbnama[i], dbharga[i])
		if err != nil {
			fmt.Println("Error inserting into database:", err)
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
    ...
    // Clear the order details after processing the order
    order.Items = nil
	dbnama = nil
	dbharga = nil
